<?php
require_once 'includes/config.php'; // Ensure path is correct
require_once 'includes/sidebar.php'; // Assuming a sidebar is included

// Initialize all required variables 
$message = ''; 
$error_message = ''; // Added to display the SQL error
$search_term = '';
$search_condition = '';
$search_params = [];
$software_items = []; 
...
if (isset($_GET['search']) && !empty($_GET['search'])) {
    ...
    // Note: We search against the alias 's' for consistency with the main query
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_software'])) {
    ...
    try {
        // FIX: Check against the new software_assignments table
...
try {
    // FIX: Calculate metric variables here to define them before use (Fixes Undefined variable warnings)
    // Total Software Titles (Correctly uses software_items)
    $total_items = $pdo->query("SELECT COUNT(software_id) FROM software_items")->fetchColumn();
    
    // Total Licenses Owned (Correctly uses software_items)
    $total_licenses_all = $pdo->query("SELECT COALESCE(SUM(total_licenses), 0) FROM software_items")->fetchColumn();
    
    // FIX: Licenses In Use - Must use the new software_assignments table
    ...
    $sql = "
        SELECT 
            ...
            -- FIX: JOIN must use the new software_assignments table name
    ...
    // FIX: Catch the SQL error and store it in $error_message
...
<head>
    ...
    <link href="includes/style.css" rel="stylesheet"> <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.min.css">
    <style>
        .custom-card {
...
<div class="d-flex" id="wrapper">
    <?php // include 'includes/sidebar.php'; // already done via require_once ?> 
    ...
    <div id="page-content-wrapper">
        <div class="container-fluid">
            ...
            <div class="d-flex justify-content-between align-items-center mb-3">
                ...
                <form class="d-flex" method="GET" action="software.php">
                    ...
                    <?php if (!empty($search_term)): ?>
                        <a href="software.php" class="btn btn-outline-danger ms-2"><i class="bi bi-x-lg"></i></a>
                    <?php endif; ?>
                </form>
            </div>
    ...
    <div class="modal-content">
      <form method="POST" action="software.php">
        ...
        <div class="modal-body">
            ...
            <input type="hidden" id="edit_licenses_in_use" name="edit_licenses_in_use"> <p class="alert alert-info small">Currently **<span id="current_in_use_display">0</span>** licenses are in use. Total licenses cannot be set below this number.</p>
